...
{
	private final static Logger LOGGER = LoggerFactory.getLogger(AdafruitMotorHat.class);
	
    private final static int HAT_ADDR = 0x60;
    private final static int DEFAULT_FREQ = 1600;
    
    private final static short PWM_HIGH = 4096;
    private final static short PWM_LOW = 0;
    ...
    void setPin(int pin, int value) throws IOException
    {
        ...
        if (value == 0)
            this.pwm.setPWM(pin, PWM_LOW, PWM_HIGH);
        if (value == 1)
            this.pwm.setPWM(pin, PWM_HIGH, PWM_LOW);
    }
    ...
    public void addStepper(int num) throws MotorException {
        ...
        {
            LOGGER.warn("hat can't add invalid stepper");
        }
    }
